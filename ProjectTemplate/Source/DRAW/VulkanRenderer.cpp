...
#pragma endregion
	...
	void Update_VulkanRenderer(entt::registry& registry, entt::entity entity)
	{
		...
		auto drawInstances = registry.group<GeometryData>(
			entt::get<GPUInstance>,
			entt::exclude<DoNotRender>
		);
		drawInstances.sort<GeometryData>([](const GeometryData& a, const GeometryData& b)
			{
				return a < b;
			});

		std::vector<GPUInstance> instanceList;
		std::map<GeometryData, int> geoInstances;
		for (auto meshEnt : drawInstances)
		{
			instanceList.push_back(registry.get<GPUInstance>(meshEnt));
			geoInstances[registry.get<GeometryData>(meshEnt)]++;
		}
		...
		if (registry.all_of< VulkanVertexBuffer, VulkanIndexBuffer>(entity))
		{
			...
			registry.emplace<std::vector<GPUInstance>>(entity, instanceList);
		...
		UpdateUIOverlays(registry, 2, overlay, blitter, font, windowWidth, windowHeight);
